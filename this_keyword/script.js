...
const userManager = {
  users: [],
  userId: 0, // Add a counter for unique ids
  init: function () {
    form.addEventListener("submit", this.submitForm.bind(this)); // we bind usermanager coz inside eventListener this referse the form element
  },
  submitForm: function (e) {
    e.preventDefault();
    this.addUser(); // here we bind this to userManger
  },
  addUser: function () {
    this.users.push({
      id:++this.userId,
   ...
   renderUi: function () {
    ...
    this.users.forEach((user) => {
      ...
      delBtn.className =
        "mt-4 px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 transition";
      delBtn.onclick = () => this.removeUser(user.id);
